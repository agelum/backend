...
export function initializeReactiveClient(
  ...
  console.log(
    "🚀 Initializing reactive client for org:",
    organizationId,
  );

  if (globalClientManager) {
    console.log(
      "⚠️ Client already initialized, cleaning up previous instance",
    );
...
export function useReactive<T = any>(
  ...
  useEffect(() => {
    ...
    const cleanup =
      clientManager.registerActiveHook(
        effectiveKey,
        [],
      );
    return cleanup;
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      try {
        ...
        const cached =
          clientManager.getCachedData(
            effectiveKey,
          );
        // Reduced logging - only log cache misses
        if (!cached) {
          console.log(
            `🔍 [useReactive] Cache miss for ${queryKey}`,
          );
        }
        ...
        if (cached) {
          try {
            ...
            const minRevalidationTime =
              5 * 60 * 1000; // 5 minutes minimum between revalidations

            // Reduced logging - only log when cache is stale
            if (cached.isStale) {
              console.log(
                `📊 [useReactive] Cache is stale for ${queryKey}`,
              );
            }
        ...
        console.log(
          `[useReactive] No cache for ${queryKey}, fetching...`,
        );

        const result =
          await clientManager.revalidateQuery(
            effectiveKey,
          );
        if (result !== undefined) {
          setData(result);
 ...
 */
export function useReactivePriorities(
  priorities: string[],
): void {
  useEffect(() => {
    // Reduced logging - only log when there are issues
...
export function useReactiveRefresh() {
  ...
  return useCallback(async () => {
    ...
    const activeHooks =
      clientManager.getStorageStats()
        ?.queries || {};
    const activeQueryKeys = Object.keys(
      activeHooks,
    );
    ...
    await Promise.allSettled(
      activeQueryKeys.map((queryKey) =>
        clientManager.revalidateQuery(
          queryKey,
        ),
      ),
    );
  ...
  useEffect(() => {
    const updateStats = () => {
      const revalidationStats =
        clientManager.getRevalidationStats();
...
export function useReactiveInvalidation(
  ...
  useEffect(() => {
    ...
    console.log(
      "[useReactiveInvalidation] Setting up invalidation listener",
    );

    return () => {
      console.log(
        "[useReactiveInvalidation] Cleaning up invalidation listener",
      );
    };
...
>(
  ...
  const executeQuery = useCallback(
    async (
      ...
      try {
        console.log(
          `[useReactiveQuery] Executing ${queryKey}`,
          variables,
        );
        ...
        console.log(
          `✅ [useReactiveQuery] ${queryKey} completed successfully`,
        );
        ...
        setError(error);
        console.error(
          `❌ [useReactiveQuery] ${queryKey} failed:`,
          error,
        );
        throw error;
      } finally {
...
export function useReactiveConnection() {
  ...
  useEffect(() => {
    const updateStatus = () => {
      setStatus(
        clientManager.getConnectionStatus(),
      );
    };
